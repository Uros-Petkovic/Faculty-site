    ...
    <body>
        ...
        <div id="sadrzaj">
            ...
            <div id="predmetdesno">
                ...
                $result= mysqli_query($con, "select * from materijal where sifra_predmet='".$_GET['sifra']."' and tip_materijala='Ispitna pitanja'");
                if(mysqli_num_rows($result)>0){
                    ...
                    echo"<span style='color:red'>Nema materijala ispitnih pitanja za ovaj predmet!</span>";
                }
                if(isset($_POST['prikazi'])){
                if($_POST['sortiranje']=='naziv_opadajuce'){ //Po izboru sortiranja prikazujemo materijale
                        $sort='naziv_opadajuce';
                        $result= mysqli_query($con, "select * from materijal join korisnik on materijal.id_nastavnik=korisnik.email where materijal.sifra_predmet='".$_GET['sifra']."' and materijal.tip_materijala='Ispitna pitanja' ORDER BY naslov DESC");
                    }elseif($_POST['sortiranje']=='naziv_rastuce'){
                        $sort='naziv_rastuce';
                        $result= mysqli_query($con, "select * from materijal join korisnik on materijal.id_nastavnik=korisnik.email where materijal.sifra_predmet='".$_GET['sifra']."' and materijal.tip_materijala='Ispitna pitanja' ORDER BY naslov ASC");
                    }elseif($_POST['sortiranje']=='datum_opadajuce'){
                        $sort='datum_opadajuce';
                        $result= mysqli_query($con, "select * from materijal join korisnik on materijal.id_nastavnik=korisnik.email where materijal.sifra_predmet='".$_GET['sifra']."' and materijal.tip_materijala='Ispitna pitanja' ORDER BY datum_objave DESC");
                    }elseif($_POST['sortiranje']=='datum_rastuce'){
                        $sort='datum_rastuce';
                        $result= mysqli_query($con, "select * from materijal join korisnik on materijal.id_nastavnik=korisnik.email where materijal.sifra_predmet='".$_GET['sifra']."' and materijal.tip_materijala='Ispitna pitanja' ORDER BY datum_objave ASC");
                    }else{
                        $sort='';
                        $result= mysqli_query($con, "select * from materijal join korisnik on materijal.id_nastavnik=korisnik.email where materijal.sifra_predmet='".$_GET['sifra']."' and materijal.tip_materijala='Ispitna pitanja'");
                    }
                    ?>
                <table>
                    <tr id="trr">
                    ...
                    mysqli_free_result($result); //Oslobadjanje resurasa
                } //Brisanje selektovanog materijala
                $result= mysqli_query($con, "select * from materijal where sifra_predmet='".$_GET['sifra']."' and tip_materijala='Ispitna pitanja'");
                while($row= mysqli_fetch_assoc($result)){
                    $id=$row['id_materijal'];
                    if(isset($_POST[$id])){
                        ...
                        if(!$result1){
                            ...
                            $s1='Location:ispitnapitanja.php?sifra='; //Povratak na stranicu za ispitna pitanja
                            $s2=$_GET['sifra'];
                            $s=$s1.$s2;
                            header($s);
                        }
